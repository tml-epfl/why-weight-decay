...
import sys
from models import get_models_scale_inv
...
model = get_models_scale_inv.get_model(config.model, n_cls, config.half_prec, get_dataset.shapes_dict[config.dataset], config.model_width,
                             batch_norm=config.batch_norm, freeze_last_layer=True, learnable_bn=False).to(memory_format=torch.channels_last).cuda()
...
with tqdm(range(start_epoch, config.epochs), desc=f'Epochs', unit='epoch') as tepoch:
    for ep in tepoch:
        ...
        aim_writer.track({'learning rate': opt.param_groups[0]['lr'], 'lambda': opt.param_groups[0]['weight_decay']}, context={
                         'subset': 'SDE'}, step=ep)
        get_models_scale_inv.bn_update(loaders['train'], model_ema)
...
get_models_scale_inv.save_model(
